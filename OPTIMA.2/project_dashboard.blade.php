...
<head>
    ...
    <title>Dashboard - {{ config('app.name', 'OPTIMA') }}</title>
...
<body class="m-0 p-0 text-white w-full overflow-x-hidden">
    ...
    <div id="dashboard-page"
        ...
        {{-- Success Message --}}
        @if (session('success'))
            <div class="bg-green-500/20 border border-green-500 text-green-200 px-4 py-3 rounded-lg mb-4 max-w-[1400px] mx-auto">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div
            ...
            <div
                ...
                <div class="flex justify-between items-center mb-5">
                    ...
                    <a href="{{ route('tasks.monitoring') }}" class="text-[#1abc9c] text-sm no-underline hover:underline">View
                        All</a>
                </div>
                <div class="flex flex-col gap-[15px]">
                    @forelse($myProjects ?? [] as $project)
                        <div class="p-[15px] bg-[#f8f9fa] rounded-lg border-l-4 
                            {{ $project->priority === 'High' ? 'border-[#e74c3c]' : '' }}
                            {{ $project->priority === 'Medium' ? 'border-[#f1c40f]' : '' }}
                            {{ $project->priority === 'Low' ? 'border-[#1abc9c]' : '' }}">
                            <h4 class="m-[0_0_5px_0] text-[#2c3e50]">{{ $project->name }}</h4>
                            <p class="m-0 text-[13px] text-[#666]">
                                Due: {{ $project->due_date ? $project->due_date->format('M d, Y') : 'N/A' }}
                            </p>
                        </div>
                    @empty
                ...
                <div class="flex justify-around items-center flex-wrap gap-5 max-sm:gap-[10px]">
                    <div class="text-center">
                        <svg viewBox="0 0 36 36" class="block mx-auto my-0 max-w-[80%] max-h-[250px] max-sm:max-w-full"
                            ...
                            <path class="circle fill-none stroke-[2.8] rounded-[round]" 
                                stroke-dasharray="{{ $teamPerformance['completion'] ?? 85 }}, 100"
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                            <text x="18" y="21" class="fill-[#666] font-sans font-bold text-[0.5em]"
                                text-anchor="middle" dominant-baseline="middle">{{ $teamPerformance['completion'] ?? 85 }}%</text>
                        </svg>
                    ...
                    <div class="text-center">
                        <svg viewBox="0 0 36 36" class="block mx-auto my-0 max-w-[80%] max-h-[250px] max-sm:max-w-full"
                            ...
                            <path class="fill-none stroke-[#eee] stroke-[3.8]"
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                            <path class="circle fill-none stroke-[2.8] rounded-[round]" 
                                stroke-dasharray="{{ $teamPerformance['onTime'] ?? 92 }}, 100"
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                            <text x="18" y="21" class="fill-[#666] font-sans font-bold text-[0.5em]"
                                text-anchor="middle" dominant-baseline="middle">{{ $teamPerformance['onTime'] ?? 92 }}%</text>
                        </svg>
